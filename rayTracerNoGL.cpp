...
__device__ __host__ float3 min3(const float3 &v1, const float3 &v2, const float3 &v3){
...
//for bounding box creation
__device__ __host__ float3 max3(const float3 &v1, const float3 &v2, const float3 &v3) {
	float3 max(v1);
	if (v2.x > max.x){
		max.x = v2.x;
	}
